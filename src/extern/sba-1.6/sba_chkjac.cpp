...
inline static void *emalloc_(char *file, int line, size_t sz) {
	...
	ptr = (void *) malloc(sz);
	if (ptr == NULL) {
		fprintf(stderr, "SBA: memory allocation request for %lu bytes failed in file %s, line %d, exiting", sz, file,
				line);
		exit(1);
	}
 ...
 *     all A_ij, B_ij and that row idx1 of jac corresponds to the gradient of the idx1-th
 ...
 *     no severe loss of significance, then if err[idx1] is 1.0 the
 *     idx1-th gradient is correct, while if err[idx1] is 0.0 the idx1-th
 *     gradient is incorrect. For values of err between 0.0 and 1.0,
 *     the categorization is less certain. In general, a value of
 *     err[idx1] greater than 0.5 indicates that the idx1-th gradient is
 *     probably correct, while a value of err[idx1] less than 0.5
 *     indicates that the idx1-th gradient is probably incorrect.
 *
...
void sba_motstr_chkjac_x(void(*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx,
		...
		struct sba_crsm *idxij, int *rcidxs, int *rcsubs, int ncon, int mcon, int cnp, int pnp, int mnp,
	...
	pb = p + m * cnp;
	for (i = 0; i < n; ++i) {
		nnz = sba_crsm_row_elmidxs(idxij, i, rcidxs, rcsubs); /* find nonzero A_ij, B_ij, idx2=0...m-1, actual column numbers in rcsubs */
		for (j = 0; j < nnz; ++j) {
	...
	for (i = 0; i < n; ++i) {
		nnz = sba_crsm_row_elmidxs(idxij, i, rcidxs, rcsubs); /* find nonzero err_ij, idx2=0...m-1 */
		for (j = 0; j < nnz; ++j) {
			...
			for (ii = 0; ii < mnp; ++ii)
				if (ptr1[ii] <= 0.5) {
					fprintf(
							stderr,
							"SBA: gradient %d (corresponding to element %d of the projection of point %d on camera %d) is %s (err=%g)\n",
							idxij->val[rcidxs[j]] * mnp + ii, ii, i, rcsubs[j], (ptr1[ii] == 0.0) ? "wrong"
									: "probably wrong", ptr1[ii]);
					++numerr;
				}
		}
	}
...
void sba_mot_chkjac_x(
		void(*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata), void(*jacf)(
				double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *jac, void *adata), double *p,
...
for(idx1=ncon; idx1<n; ++idx1)
for(idx2=mcon; idx2<m; ++idx2) {
	if(!vmask[idx1*m+idx2]) continue; // point idx1 does not appear in image idx2

	sba_motstr_chkjac(proj, projac, p+idx2*cnp, p+m*cnp+idx1*pnp, idx2, idx1, cnp, pnp, mnp, adata, adata);
}
 ...
 *     jacobian of proj at p. Note that row idx1 of jac corresponds to the gradient of the idx1-th
 *     component of proj, evaluated at p.
 * aj, bi are input arrays of lengths cnp, pnp containing the parameters for the point of
 *     evaluation, idx1.e. idx2-th camera and idx1-th point
		...
		union proj_projac *funcs, double *aj, double *bi, int jj, int ii, int cnp, int pnp, int mnp, void *func_adata, void *jac_adata)
{
	...
	for(idx2=0; idx2<cnp; ++idx2) {
		temp=eps*FABS(aj[idx2]);
		if(temp==zero) temp=eps;
		ajp[idx2]=aj[idx2]+temp;
	}
	for(idx2=0; idx2<pnp; ++idx2) {
		temp=eps*FABS(bi[idx2]);
		if(temp==zero) temp=eps;
		bip[idx2]=bi[idx2]+temp;
	}
	...
	for(idx2=0; idx2<cnp; ++idx2) {
		...
		for(idx1=0; idx1<mnp; ++idx1)
		err[idx1]+=temp*Aij[idx1*cnp+idx2];
	}
	for(idx2=0; idx2<pnp; ++idx2) {
		temp=FABS(bi[idx2]);
		if(temp==zero) temp=one;
		...
		for(idx1=0; idx1<mnp; ++idx1)
		err[idx1]+=temp*Bij[idx1*pnp+idx2];
	}

	for(idx1=0; idx1<mnp; ++idx1) {
		temp=one;
		if(fvec[idx1]!=zero && fvecp[idx1]!=zero && FABS(fvecp[idx1]-fvec[idx1])>=epsf*FABS(fvec[idx1]))
		temp=eps*FABS((fvecp[idx1]-fvec[idx1])/eps - err[idx1])/(FABS(fvec[idx1])+FABS(fvecp[idx1]));
		err[idx1]=one;
		if(temp>epsmch && temp<eps)
		err[idx1]=(log10(temp) - epslog)/epslog;
		if(temp>=eps) err[idx1]=zero;
	}

	for(idx1=0; idx1<mnp; ++idx1)
	if(err[idx1]<=0.5) {
		fprintf(stderr, "SBA: gradient %d (corresponding to element %d of the projection of point %d on camera %d) is %s (err=%g)\n",
				idx1, idx1, ii, jj, (err[idx1]==0.0)? "wrong" : "probably wrong", err[idx1]);
		++numerr;
	}
