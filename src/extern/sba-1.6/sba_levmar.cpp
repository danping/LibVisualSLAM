...
inline static void *emalloc_(char *file, int line, size_t sz) {
	...
	ptr = (void *) malloc(sz);
	if (ptr == NULL) {
		fprintf(stderr, "SBA: memory allocation request for %u bytes failed in file %s, line %d, exiting", sz, file,
				line);
		exit(1);
	}
...
static double sba_mean_repr_error(int n,
		int mnp,
		double *x,
		double *hx,
...
static void sba_print_sol(int n,
		int m,
		double *p,
		int cnp,
		int pnp,
 ...
 */
static double nrmCxmy(double * const e,
 ...
 */
static void sba_fdjac_x(double *p, /* I: current parameter estimate, (m*cnp+n*pnp)x1 */
...
typedef int (*PLS)(double *A, double *B, double *x, int m, int iscolmaj, double** pbuf, int* pbuf_sz);
...
int sba_motstr_levmar_x(const int n, /* number of points */
 ...
 */
	...
	double *wght =
			/* work array for storing the weights computed from the covariance inverses, max. size n*m*mnp*mnp */NULL;
	...
	*pdp; /* p + dp, size m*cnp + n*pnp */
	...
	double tau = FABS(opts[0]), eps1 = FABS(opts[1]), eps2 = FABS(opts[2]), eps2_sq = opts[2] * opts[2], eps3_sq =
			opts[3] * opts[3], eps4_sq = opts[4] * opts[4];
	double init_p_eL2;
	int nu = 2, nu2, stop = 0, nfev, njev = 0, nlss = 0;
	int nobs, nvars;
	const int mmcon = m - mcon;
	PLS linsolver = NULL;
	int (*matinv)(double *A, int m, double** pbuf, int* pbuf_sz, int* pnb) = NULL;
	...
	for (itno = 0; itno < itmax && !stop; ++itno) {
		...
		while (1) {
			...
			for (i = ncon; i < n; ++i) {
				...
				j = sba_symat_invert_BK(ptr1, pnp, &bk_buf, &bk_buf_sz, &bk_nb);
				matinv = sba_symat_invert_BK;
				if (!j) {
					fprintf(stderr, "SBA: singular matrix V*_i (i=%d) in sba_motstr_levmar_x(), increasing damping\n",
							i);
					goto moredamping;
					// increasing damping will eventually make V*_i diagonally dominant, thus nonsingular
					//retval=SBA_ERROR;
					//goto freemem_and_return;
				}
			}
...
#if 0
			if(verboseLM>1) { /* count the nonzeros in S */
			...
			issolved = sba_Axb_Chol(S, E + mcon * cnp, dpa + mcon * cnp, Sdim, MAT_STORAGE, &chol_buf, &chol_buf_sz);
			...
			if (issolved) {
				...
				if (dp_L2 >= (p_L2 + eps2) / SBA_EPSILON_SQ) { /* almost singular */
					...
					fprintf(stderr,
							"SBA: the matrix of the augmented normal equations is almost singular in sba_motstr_levmar_x(),\n"
			...
			if (nu2 <= nu) { /* nu has wrapped around (overflown) */
				fprintf(stderr,
						"SBA: too many failed attempts to increase the damping factor in sba_motstr_levmar_x()! Singular Hessian matrix?\n");
				//exit(1);
				stop = 6;
				break;
			}
	...
	if (matinv)
		(*matinv)(NULL, 0, &bk_buf, &bk_buf_sz, &bk_nb);
	if (linsolver)
		(*linsolver)(NULL, NULL, NULL, 0, 0, &chol_buf, &chol_buf_sz);
...
int sba_mot_levmar_x(const int n, /* number of points */
const int m, /* number of images */
const int mcon,/* number of images (starting from the 1st) whose parameters should not be modified.
 * All A_ij (see below) with j<mcon are assumed to be zero
 */
	...
	for (itno = 0; itno < itmax && !stop; ++itno) {
		...
		while (1) {
			...
			for (j = nsolved = mcon; j < m; ++j) {
				...
				nsolved += sba_Axb_Chol(ptr1, ptr2, ptr3, cnp, 0, &chol_buf, &chol_buf_sz);
				linsolver = sba_Axb_Chol;
			...
			if (nu2 <= nu) { /* nu has wrapped around (overflown) */
				fprintf(stderr,
						"SBA: too many failed attempts to increase the damping factor in sba_mot_levmar_x()! Singular Hessian matrix?\n");
				//exit(1);
				stop = 6;
				break;
			}
...
int sba_str_levmar_x(const int n, /* number of points */
const int ncon,/* number of points (starting from the 1st) whose parameters should not be modified.
 * All B_ij (see below) with i<ncon are assumed to be zero
 */
const int m, /* number of images */
	...
	for (itno = 0; itno < itmax && !stop; ++itno) {
		...
		while (1) {
			...
			for (i = nsolved = ncon; i < n; ++i) {
				...
				nsolved += sba_Axb_Chol(ptr1, ptr2, ptr3, pnp, 0, &chol_buf, &chol_buf_sz);
				linsolver = sba_Axb_Chol;
			...
			if (nu2 <= nu) { /* nu has wrapped around (overflown) */
				fprintf(stderr,
						"SBA: too many failed attempts to increase the damping factor in sba_str_levmar_x()! Singular Hessian matrix?\n");
				//exit(1);
				stop = 6;
				break;
			}
...
//add by Zou Danping
int sba_mot_levmar_wx(const int n, /* number of points */
const int m, /* number of images */
const int mcon,/* number of images (starting from the 1st) whose parameters should not be modified.
 ...
 */
double *weightx, /* measurements weight matrices, 
 * When set to NULL, all weights are implicitely set to one in this case).
 * see vmask[idx1, idx2], max. size n*m
 */
const int mnp,/* number of parameters for EACH measurement; usually 2 */
void(*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata),
/* functional relation describing measurements. Given a parameter vector p,
 ...
 */
	...
	struct sba_crsm idxij; /* sparse matrix containing the location of x_ij in x. This is also the location of A_ij 
	 * in jac, e_ij in e, etc.
	 * This matrix can be thought as a map from a sparse set of pairs (idx1, idx2) to a continuous
	 * index k and it is used to efficiently lookup the memory locations where the non-zero
	 * blocks of a sparse matrix/vector are stored
	 */
	int maxCPvis, /* max. of projections across cameras & projections across points */
	...
	*pdp; /* p + dp, size m*cnp */
	...
	if (weightx != NULL) {
		for (i = 0; i < n; ++i) {
			nnz = sba_crsm_row_elmidxs(&idxij, i, rcidxs, rcsubs); /* find nonzero x_ij, idx2=0...m-1 */
			for (j = 0; j < nnz; ++j) {
				/* set ptr1, ptr2 to point to cov_x_ij, w_x_ij resp. */
				//ptr1 = weightx + (k = idxij.val[rcidxs[j]] * covsz);
				//Modified by Danping Zou
				k = idxij.val[rcidxs[j]];
				double wt = sqrt(weightx[k]);
				ptr2 = wght + k * covsz;
				//if (!sba_mat_cholinv(ptr1, ptr2, mnp)) { /* compute w_x_ij s.t. w_x_ij^T w_x_ij = cov_x_ij^-1 */
				if (wt < 0) {
					fprintf(stderr, "SBA: invalid weight for x_ij (i=%d, j=%d) in sba_motstr_levmar_x()\n", i,
							rcsubs[j]);
					retval = SBA_ERROR;
					goto freemem_and_return;
				}
	...
	for (itno = 0; itno < itmax && !stop; ++itno) {
		...
		for (j = mcon; j < m; ++j) {
			...
			for (i = 0; i < nnz; ++i) {
				/* set ptr3 to point to A_ij, actual row number in rcsubs[idx1] */
				ptr3 = jac + idxij.val[rcidxs[i]] * Asz;
...
#if 0
			/* restore U diagonal entries */
			for(idx2=mcon; idx2<m; ++idx2) {
				ptr1=U + idx2*Usz; // set ptr1 to point to U_j
				ptr2=diagU + idx2*cnp; // set ptr2 to point to diagU_j
				for(idx1=0; idx1<cnp; ++idx1)
				ptr1[idx1*cnp+idx1]=ptr2[idx1];
			}
#endif
		} /* inner while loop */
 ...
 * Returns the number of iterations (>=0) if successfull, SBA_ERROR if failed
 */
//revised by Danping Zou
int sba_motstr_levmar_wx(const int n, /* number of points */
const int ncon,/* number of points (starting from the 1st) whose parameters should not be modified.
 * All B_ij (see below) with i<ncon are assumed to be zero
 */
const int m, /* number of images */
 ...
 * see vmask[i, j], max. size n*m*mnp
 */
double *weightx, /* weight for each measurement*/
const int mnp,/* number of parameters for EACH measurement; usually 2 */
void(*func)(double *p, struct sba_crsm *idxij, int *rcidxs, int *rcsubs, double *hx, void *adata),
/* functional relation describing measurements. Given a parameter vector p,
 * computes a prediction of the measurements \hat{x}. p is (m*cnp + n*pnp)x1,
 ...
 */
	...
	*pdp; /* p + dp, size m*cnp + n*pnp */
	...
	int nobs1 = nvis * mnp;
	int nvars1 = (m - mcon) * cnp + (n - ncon) * pnp;
