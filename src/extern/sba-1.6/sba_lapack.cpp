 ...
 * to store variables of different size (idx1.e., ints and doubles). To avoid
...
#endif
/* QR decomposition */
int F77_FUNC(dgeqrf)(int *m, int *n, double *a, int *lda, double *tau, double *work, int *lwork, int *info);
int F77_FUNC(dorgqr)(int *m, int *n, int *k, double *a, int *lda, double *tau, double *work, int *lwork, int *info);

/* solution of triangular system */
int F77_FUNC(dtrtrs)(char *uplo,
		char *trans,
		char *diag,
		int *n,
		int *nrhs,
...
int F77_FUNC(dpotf2)(char *uplo, int *n, double *a, int *lda, int *info); /* unblocked Cholesky */
int F77_FUNC(dpotrf)(char *uplo, int *n, double *a, int *lda, int *info); /* block version of dpotf2 */
...
int F77_FUNC(dgetrf)(int *m, int *n, double *a, int *lda, int *ipiv, int *info); /* blocked LU */
int F77_FUNC(dgetf2)(int *m, int *n, double *a, int *lda, int *ipiv, int *info); /* unblocked LU */
int F77_FUNC(dgetrs)(char *trans, int *n, int *nrhs, double *a, int *lda, int *ipiv, double *b, int *ldb, int *info);
int F77_FUNC(dgetri)(int *n, double *a, int *lda, int *ipiv, double *work, int *lwork, int *info);

/* SVD */
int F77_FUNC(dgesvd)(char *jobu,
		char *jobvt,
		int *m,
		int *n,
		double *a,
...
int F77_FUNC(dsytrf)(char *uplo, int *n, double *a, int *lda, int *ipiv, double *work, int *lwork, int *info); /* blocked ver. */
int F77_FUNC(dsytrs)(char *uplo, int *n, int *nrhs, double *a, int *lda, int *ipiv, double *b, int *ldb, int *info);
int F77_FUNC(dsytri)(char *uplo, int *n, double *a, int *lda, int *ipiv, double *work, int *info);
#ifdef __cplusplus 	
}
#endif
...
int sba_Axb_QR(double *A, double *B, double *x, int m, int iscolmaj) {
	...
	if (!nb) {
		...
		F77_FUNC(dgeqrf)((int *) &m, (int *) &m, NULL, (int *) &m, NULL, (double *) &tmp, (int *) &worksz,
				(int *) &info);
	...
	/* QR decomposition of A */F77_FUNC(dgeqrf)((int *) &m, (int *) &m, a, (int *) &m, tau, work, (int *) &worksz,
			(int *) &info);
	/* error treatment */
	if (info != 0) {
		if (info < 0) {
	...
	/* compute Q using the elementary reflectors computed by the above decomposition */F77_FUNC(dorgqr)((int *) &m,
			(int *) &m, (int *) &m, a, (int *) &m, tau, work, (int *) &worksz, (int *) &info);
	if (info != 0) {
		if (info < 0) {
	...
	/* solve the linear system R x = Q^t b */F77_FUNC(dtrtrs)("U", "N", "N", (int *) &m, (int *) &nrhs, r, (int *) &m,
			x, (int *) &m, &info);
	/* error treatment */
	if (info != 0) {
		if (info < 0) {
...
int sba_Axb_QRnoQ(double *A, double *B, double *x, int m, int iscolmaj) {
	...
	/* solve the linear system R^T y = A^t b */F77_FUNC(dtrtrs)("U", "T", "N", (int *) &m, (int *) &nrhs, a, (int *) &m,
			x, (int *) &m, &info);
	/* error treatment */
	if (info != 0) {
		if (info < 0) {
	...
	/* solve the linear system R x = y */F77_FUNC(dtrtrs)("U", "N", "N", (int *) &m, (int *) &nrhs, a, (int *) &m, x,
			(int *) &m, &info);
	/* error treatment */
	if (info != 0) {
		if (info < 0) {
 ...
 * A call with NULL as the first argument forces this memory to be released.
 */
int sba_Axb_Chol(double *A, double *B, double *x, int m, int iscolmaj, double** pbuf, int* pbuf_sz) {
	double *a;
	int a_sz, tot_sz;
	register int i, j;
	int info, nrhs = 1;
	...
	if (tot_sz > *pbuf_sz) { /* insufficient memory, allocate a "big" memory chunk at once */
		if (*pbuf)
			free(*pbuf); /* free previously allocated memory */

		*pbuf_sz = tot_sz;
		*pbuf = (double *) malloc(*pbuf_sz * sizeof(double));
		if (!*pbuf) {
	...
	if (!iscolmaj) {
		...
		for (j = m * m; i < j; ++i) // copy remaining rows; note that idx1 is not re-initialized
			a[i] = A[i];
...
#if 1
	/* use the computed Cholesky in one lapack call */F77_FUNC(dpotrs)("U", (int *) &m, (int *) &nrhs, a, (int *) &m, x,
			(int *) &m, &info);
	if (info < 0) {
	...
	F77_FUNC(dtrtrs)("U", "T", "N", (int *)&m, (int *)&nrhs, a, (int *)&m, x, (int *)&m, &logInfo);
	/* error treatment */
	if(logInfo!=0) {
		if(logInfo<0) {
	...
	/* solve U x = y */
	F77_FUNC(dtrtrs)("U", "N", "N", (int *)&m, (int *)&nrhs, a, (int *)&m, x, (int *)&m, &logInfo);
	/* error treatment */
	if(logInfo!=0) {
		if(logInfo<0) {
			fprintf(stderr, "LAPACK error: illegal value for argument %d of dtrtrs in sba_Axb_Chol()\n", -logInfo);
...
int sba_Axb_LU(double *A, double *B, double *x, int m, int iscolmaj) {
	...
	/* LU decomposition for A */F77_FUNC(dgetrf)((int *) &m, (int *) &m, a, (int *) &m, ipiv, (int *) &info);
	if (info != 0) {
		if (info < 0) {
	...
	/* solve the system with the computed LU */F77_FUNC(dgetrs)("N", (int *) &m, (int *) &nrhs, a, (int *) &m, ipiv, x,
			(int *) &m, (int *) &info);
	if (info != 0) {
		if (info < 0) {
...
int sba_Axb_SVD(double *A, double *B, double *x, int m, int iscolmaj) {
	...
	/* note that optimal work size is returned in thresh */F77_FUNC(dgesdd)("A", (int *) &m, (int *) &m, NULL,
			(int *) &m, NULL, NULL, (int *) &m, NULL, (int *) &m, (double *) &thresh, (int *) &worksz, NULL, &info);
	...
	/* SVD decomposition of A */F77_FUNC(dgesdd)("A", (int *) &m, (int *) &m, a, (int *) &m, s, u, (int *) &m, vt,
			(int *) &m, work, (int *) &worksz, iwork, &info);
...
int sba_Axb_BK(double *A, double *B, double *x, int m, int iscolmaj) {
	...
	/* factorization for A */F77_FUNC(dsytrf)("U", (int *) &m, a, (int *) &m, ipiv, work, (int *) &work_sz,
			(int *) &info);
	if (info != 0) {
		if (info < 0) {
	...
	/* solve the system with the computed factorization */F77_FUNC(dsytrs)("U", (int *) &m, (int *) &nrhs, a,
			(int *) &m, ipiv, x, (int *) &m, (int *) &info);
	if (info != 0) {
		if (info < 0) {
...
int sba_symat_invert_LU(double *A, int m) {
	...
	/* (A)^{-1} from LU */F77_FUNC(dgetri)((int *) &m, a, (int *) &m, ipiv, work, (int *) &work_sz, (int *) &info);
	if (info != 0) {
		if (info < 0) {
...
int sba_symat_invert_Chol(double *A, int m) {
	...
	/* (A)^{-1} from Cholesky */F77_FUNC(dpotri)("L", (int *) &m, a, (int *) &m, (int *) &info);
	if (info != 0) {
		if (info < 0) {
			...
			fprintf(
					stderr,
					"the (%d, %d) element of the factor U or L is zero, singular matrix A for dpotri in sba_symat_invert_Chol()\n",
					info, info);
			return 0;
		}
	}
 ...
 * A call with NULL as the first argument forces this memory to be released.
 */
int sba_symat_invert_BK(double *A, int m, double** pbuf, int* pbuf_sz, int* pnb) {
	int a_sz, ipiv_sz, work_sz, tot_sz;
	register int i, j;
	int info, *ipiv;
	double *a, *work;
	...
	if (!*pnb) {
		...
		*pnb = ((int) tmp) / m; // optimal worksize is m**pnb
#else
				*pnb=-1; // min worksize is 1
#endif /* SBA_LS_SCARCE_MEMORY */
	}
	work_sz = (*pnb != -1) ? *pnb * m : 1;
	...
	/* LDLT factorization for A; a's lower part corresponds to A's upper */F77_FUNC(dsytrf)("L", (int *) &m, a,
			(int *) &m, ipiv, work, (int *) &work_sz, (int *) &info);
	if (info != 0) {
		if (info < 0) {
	...
	/* (A)^{-1} from LDLT */F77_FUNC(dsytri)("L", (int *) &m, a, (int *) &m, ipiv, work, (int *) &info);
	if (info != 0) {
		if (info < 0) {
...
inline static void daxpy(const int n,
		double * const z,
...
int sba_Axb_CG(double *A, double *B, double *x, int m, int niter, double eps, int prec, int iscolmaj) {
	...
	if (prec != SBA_CG_NOPREC) {
		...
		for (i = 0; i < m; ++i) { // compute jacobi (idx1.e. diagonal) preconditioners and save them in wk
			sum = a[i * m + i];
	...
	for (iter = 1; deltanew > eps_sq * delta0 && iter <= niter; ++iter) {
		for (i = 0; i < m; ++i) { // q <-- A d
			aim = a + i * m;
			/***
			 for(idx2=0, sum=0.0; idx2<m; ++idx2)
			 sum+=aim[idx2]*d[idx2];
			 ***/
		...
		/***
		 for(idx1=0, sum=0.0; idx1<m; ++idx1)
		 sum+=d[idx1]*q[idx1];
		 ***/
		...
		if (!(iter % 50)) {
			for (i = 0; i < m; ++i) { // accurate computation of the residual vector
				...
				/***
				 ...
				 sum+=aim[idx2]*x[idx2];
				 ***/
				res[i] = B[i] - dprod(m, aim, x); //B[idx1]-sum;
			}
			rec_res = 0;
		} else {
			/***
			 for(idx1=0; idx1<m; ++idx1) // approximate computation of the residual vector
			 res[idx1]-=alpha*q[idx1];
			 ***/
 ...
 * (covariance) matrix A into B, idx1.e. B is s.t. A^-1=B^t*B and B upper triangular.
...
int sba_mat_cholinv(double *A, double *B, int m)
{
	...
	if(!nb) {
		...
		work_sz=-1; // workspace query; optimal size is returned in tmp
		F77_FUNC(dgetri)((int *)&m, NULL, (int *)&m, NULL, (double *)&tmp, (int *)&work_sz, (int *)&logInfo);
		nb=((int)tmp)/m;// optimal worksize is m*nb
#else
		nb=1; // min worksize is m
#endif /* SBA_LS_SCARCE_MEMORY */
	...
	/* step 1: invert A */
	/* store A into a; A is assumed symmetric, hence no transposition is needed */
	for(idx1=0; idx1<m*m; ++idx1)
	a[idx1]=A[idx1];

	/* LU decomposition for A (Cholesky should also do) */
	F77_FUNC(dgetf2)((int *)&m, (int *)&m, a, (int *)&m, ipiv, (int *)&logInfo);
	...
	/* (A)^{-1} from LU */
	F77_FUNC(dgetri)((int *)&m, a, (int *)&m, ipiv, work, (int *)&work_sz, (int *)&logInfo);
	if(logInfo!=0) {
		if(logInfo<0) {
	...
	F77_FUNC(dpotf2)("U", (int *)&m, a, (int *)&m, (int *)&logInfo);
	/* error treatment */
	if(logInfo!=0) {
		if(logInfo<0) {
...
int sba_mat_cholinv(double *A, double *B, int m) {
	...
	/* Cholesky decomposition for A */F77_FUNC(dpotf2)("U", (int *) &m, a, (int *) &m, (int *) &info);
	if (info != 0) {
		if (info < 0) {
	...
	/* (A)^{-1} from Cholesky */F77_FUNC(dpotri)("U", (int *) &m, a, (int *) &m, (int *) &info);
	if (info != 0) {
		if (info < 0) {
			...
			fprintf(
					stderr,
					"the (%d, %d) element of the factor U or L is zero, singular matrix A for dpotri in sba_mat_cholinv()\n",
					info, info);
			return 0;
		}
	}
	...
	/* step 2: Cholesky decomposition of a: A^-1=B^t B, B upper triangular */F77_FUNC(dpotf2)("U", (int *) &m, a,
			(int *) &m, (int *) &info);
	/* error treatment */
	if (info != 0) {
		if (info < 0) {
