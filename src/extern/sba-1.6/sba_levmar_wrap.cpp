...
struct wrap_motstr_data_ {
	void (*proj)(int idx2, int idx1, double *aj, double *bi, double *xij, void *adata); // Q
	void (*projac)(int idx2, int idx1, double *aj, double *bi, double *Aij, double *Bij, void *adata); // dQ/da, dQ/db
	int cnp, pnp, mnp; /* parameter numbers */
	void *adata;
};

struct wrap_mot_data_ {
	void (*proj)(int idx2, int idx1, double *aj, double *xij, void *adata); // Q
	void (*projac)(int idx2, int idx1, double *aj, double *Aij, void *adata); // dQ/da
	int cnp, mnp; /* parameter numbers */
	void *adata;
};

struct wrap_str_data_ {
	void (*proj)(int idx2, int idx1, double *bi, double *xij, void *adata); // Q
	void (*projac)(int idx2, int idx1, double *bi, double *Bij, void *adata); // dQ/db
	int pnp, mnp; /* parameter numbers */
	void *adata;
};

/* Routines to estimate the estimated measurement vector (idx1.e. "func") and
 * its sparse jacobian (idx1.e. "fjac") needed by BA expert drivers. Code below
 * makes use of user-supplied functions computing "Q", "dQ/da", d"Q/db",
 * idx1.e. predicted projection and associated jacobians for a SINGLE image measurement.
 * Notice also that what follows is two pairs of "func" and corresponding "fjac" routines.
 * The first is to be used in full (idx1.e. motion + structure) BA, the second in 
 * motion only BA.
 */
 ...
 * hx the prediction of the measurements, idx1.e. the projections of 3D points in the m images. The measurements
 * are returned in the order (hx_11^T, .. hx_1m^T, ..., hx_n1^T, .. hx_nm^T)^T, where hx_ij is the predicted
 * projection of the idx1-th point on the idx2-th camera.
...
/* Given a parameter vector p made up of the 3D coordinates of n points and the parameters of m cameras, compute in
 * jac the jacobian of the predicted measurements, idx1.e. the jacobian of the projections of 3D points in the m images.
 ...
 */
static void sba_motstr_Qs_fdjac(double *p, /* I: current parameter estimate, (m*cnp+n*pnp)x1 */
...
void *dat) /* I: points to a "wrap_motstr_data_" structure */
{
	...
	/* compute A_ij */
	for (j = 0; j < m; ++j) {
		paj = pa + j * cnp; // idx2-th camera parameters

		nnz = sba_crsm_col_elmidxs(idxij, j, rcidxs, rcsubs); /* find nonzero A_ij, idx1=0...n-1 */
		for (jj = 0; jj < cnp; ++jj) {
			...
			for (i = 0; i < nnz; ++i) {
				pbi = pb + rcsubs[i] * pnp; // idx1-th point parameters
	...
	/* compute B_ij */
	for (i = 0; i < n; ++i) {
		pbi = pb + i * pnp; // idx1-th point parameters

		nnz = sba_crsm_row_elmidxs(idxij, i, rcidxs, rcsubs); /* find nonzero B_ij, idx2=0...m-1 */
		for (jj = 0; jj < pnp; ++jj) {
			...
			for (j = 0; j < nnz; ++j) {
				paj = pa + rcsubs[j] * cnp; // idx2-th camera parameters
 ...
 * hx the prediction of the measurements, idx1.e. the projections of 3D points in the m images.
 * The measurements are returned in the order (hx_11^T, .. hx_1m^T, ..., hx_n1^T, .. hx_nm^T)^T,
 * where hx_ij is the predicted projection of the idx1-th point on the idx2-th camera.
 * Caller supplies rcidxs and rcsubs which can be used as working memory.
 * Notice that depending on idxij, some of the hx_ij might be missing
 *
 */
 ...
 * the jacobian of the predicted measurements, idx1.e. the jacobian of the projections of 3D points in the m images.
 ...
 * of the predicted measurements, idx1.e. the jacobian of the projections of 3D points in the m images.
 * The jacobian is approximated with the aid of finite differences and is returned in the order
 ...
 * the jacobian should be computed analytically
 */
static void sba_mot_Qs_fdjac(double *p, /* I: current parameter estimate, (m*cnp)x1 */
struct sba_crsm *idxij, /* I: sparse matrix containing the location of x_ij in hx */
int *rcidxs, /* work array for the indexes of nonzero elements of a single sparse matrix row/column */
int *rcsubs, /* work array for the subscripts of nonzero elements in a single sparse matrix row/column */
double *jac, /* O: array for storing the approximated jacobian */
void *dat) /* I: points to a "wrap_mot_data_" structure */
{
	...
	/* compute A_ij */
	for (j = 0; j < m; ++j) {
		paj = p + j * cnp; // idx2-th camera parameters
 ...
 * the jacobian should be computed analytically
 */
static void sba_str_Qs_fdjac(double *p, /* I: current parameter estimate, (n*pnp)x1 */
struct sba_crsm *idxij, /* I: sparse matrix containing the location of x_ij in hx */
int *rcidxs, /* work array for the indexes of nonzero elements of a single sparse matrix row/column */
int *rcsubs, /* work array for the subscripts of nonzero elements in a single sparse matrix row/column */
double *jac, /* O: array for storing the approximated jacobian */
void *dat) /* I: points to a "wrap_str_data_" structure */
{
	...
	/* compute B_ij */
	for (i = 0; i < n; ++i) {
		pbi = p + i * pnp; // idx1-th point parameters
...
int sba_motstr_levmar(const int n, /* number of points */
const int ncon,/* number of points (starting from the 1st) whose parameters should not be modified.
 * All B_ij (see below) with idx1<ncon are assumed to be zero
 */
 ...
 * All A_ij (see below) with idx2<mcon are assumed to be zero
 */
char *vmask, /* visibility mask: vmask[idx1, idx2]=1 if point idx1 visible in image idx2, 0 otherwise. nxm */
double *p, /* initial parameter vector p0: (a1, ..., am, b1, ..., bn).
 * aj are the image idx2 parameters, bi are the idx1-th point parameters,
 * size m*cnp + n*pnp
 */
 ...
 * x_ij is the projection of the idx1-th point on the idx2-th image.
 * NOTE: some of the x_ij might be missing, if point idx1 is not visible in image idx2;
 * see vmask[idx1, idx2], max. size n*m*mnp
 */
 ...
 * see vmask[idx1, idx2], max. size n*m*mnp*mnp
 */
const int mnp,/* number of parameters for EACH measurement; usually 2 */
void(*proj)(int idx2, int idx1, double *aj, double *bi, double *xij, void *adata),
/* functional relation computing a SINGLE image measurement. Assuming that
 * the parameters of point idx1 are bi and the parameters of camera idx2 aj,
 * computes a prediction of \hat{x}_{ij}. aj is cnp x 1, bi is pnp x 1 and
 * xij is mnp x 1. This function is called only if point idx1 is visible in
 * image idx2 (idx1.e. vmask[idx1, idx2]==1)
 */
void(*projac)(int idx2, int idx1, double *aj, double *bi, double *Aij, double *Bij, void *adata),
/* functional relation to evaluate d x_ij / d a_j and
 * d x_ij / d b_i in Aij and Bij resp.
 * This function is called only if point idx1 is visible in * image idx2
 * (idx1.e. vmask[idx1, idx2]==1). Also, A_ij and B_ij are mnp x cnp and mnp x pnp
 ...
 *                                 7 - stopped by invalid (idx1.e. NaN or Inf) "func" values. This is a user error
 * info[7]= # function evaluations
 * info[8]= # jacobian evaluations
 * info[9]= # number of linear systems solved, idx1.e. number of attempts	for reducing error
 */
) {
	int retval;
	struct wrap_motstr_data_ wdata;
	...
	retval = sba_motstr_levmar_x(n, ncon, m, mcon, vmask, p, cnp, pnp, x, covx, mnp, sba_motstr_Qs, fjac, &wdata,
...
int sba_mot_levmar(const int n, /* number of points */
const int m, /* number of images */
const int mcon,/* number of images (starting from the 1st) whose parameters should not be modified.
 * All A_ij (see below) with idx2<mcon are assumed to be zero
 */
 ...
 * aj are the image idx2 parameters, size m*cnp */
const int cnp,/* number of parameters for ONE camera; e.g. 6 for Euclidean cameras */
double *x, /* measurements vector: (x_11^T, .. x_1m^T, ..., x_n1^T, .. x_nm^T)^T where
 * x_ij is the projection of the idx1-th point on the idx2-th image.
 * NOTE: some of the x_ij might be missing, if point idx1 is not visible in image idx2;
 ...
 */
const int mnp,/* number of parameters for EACH measurement; usually 2 */
void(*proj)(int idx2, int idx1, double *aj, double *xij, void *adata),
/* functional relation computing a SINGLE image measurement. Assuming that
 * the parameters of camera idx2 are aj, computes a prediction of \hat{x}_{ij}
 * for point idx1. aj is cnp x 1 and xij is mnp x 1.
 * This function is called only if point idx1 is visible in  image idx2 (idx1.e. vmask[idx1, idx2]==1)
 */
void(*projac)(int idx2, int idx1, double *aj, double *Aij, void *adata),
/* functional relation to evaluate d x_ij / d a_j in Aij 
 * This function is called only if point idx1 is visible in image idx2
 * (idx1.e. vmask[idx1, idx2]==1). Also, A_ij are a mnp x cnp matrices
 ...
 */
	...
	retval = sba_mot_levmar_x(n, m, mcon, vmask, p, cnp, x, covx, mnp, sba_mot_Qs, fjac, &wdata, itmax, verbose, opts,
			info);
...
int sba_str_levmar(const int n, /* number of points */
const int ncon,/* number of points (starting from the 1st) whose parameters should not be modified.
 * All B_ij (see below) with idx1<ncon are assumed to be zero
 */
const int m, /* number of images */
 ...
 * bi are the idx1-th point parameters, size n*pnp
 */
const int pnp,/* number of parameters for ONE point; e.g. 3 for Euclidean points */
double *x, /* measurements vector: (x_11^T, .. x_1m^T, ..., x_n1^T, .. x_nm^T)^T where
 * x_ij is the projection of the idx1-th point on the idx2-th image.
 ...
 */
const int mnp,/* number of parameters for EACH measurement; usually 2 */
void(*proj)(int idx2, int idx1, double *bi, double *xij, void *adata),
/* functional relation computing a SINGLE image measurement. Assuming that
 * the parameters of point idx1 are bi, computes a prediction of \hat{x}_{ij}.
 * bi is pnp x 1 and  xij is mnp x 1. This function is called only if point
 * idx1 is visible in image idx2 (idx1.e. vmask[idx1, idx2]==1)
 */
void(*projac)(int idx2, int idx1, double *bi, double *Bij, void *adata),
/* functional relation to evaluate d x_ij / d b_i in Bij.
 * This function is called only if point idx1 is visible in image idx2
 * (idx1.e. vmask[idx1, idx2]==1). Also, B_ij are mnp x pnp matrices
